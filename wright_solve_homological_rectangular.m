function a = wright_solve_homological_rectangular(alpha, lambda, A, xi_1, numCoef, n)
%WRIGHT_SOLVE_HOMOLOGICAL_RECTANGULAR - Computes a Taylor parameterization for the unstable manifold of the pseudospectral
% approximation of Wright's equation by solving the homological equations for rectangular data shape. This means compute
% coefficients satisfying |beta| <= N (see wright_solve_homological_diagonal for the alternative).
...
for beta_1 = 2:numCoef-1
    for beta_2 = 2:numCoef-1
        multiIndex = [beta_1, beta_2];
        hom_rhs = homological_rhs_rectangular(a(1), a(end), multiIndex, alpha);
        A_idx = homological_linear_map(A, lambda, multiIndex);
        rhs_idx = cat(1, hom_rhs, zeros(n, 1));
...
end % end wright_solve_homological_rectangular
